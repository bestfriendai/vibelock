...
requiredFiles.forEach((filePath, index) => {
  ...
  if (exists) {
    ...
    const hasWorkaroundImport =
      content.includes("imagePickerWorkaround") || content.includes("launchImageLibraryWithWorkaround");
...
if (fs.existsSync(workaroundPath)) {
  ...
  const checks = [
    {
      name: "launchImageLibraryWithWorkaround function",
      pattern: /export async function launchImageLibraryWithWorkaround/,
    },
...
const componentChecks = [
  {
    ...
    checks: [
      {
        name: "Imports workaround utility",
        pattern: /import.*launchImageLibraryWithWorkaround.*from.*imagePickerWorkaround/,
      },

...
class ProductionTestRunner {
  ...
  async testStartupPerformance() {
    try {
      ...
      // Check for Hermes
      const buildProperties = config.expo?.plugins?.find(plugin => 
  ...
  async runMemoryLeakDetection() {
    ...
    try {
      ...
      const leakPatterns = [
        ...
        { pattern: /addEventListener\(/g, cleanup: /removeEventListener\(/g, name: 'addEventListener without removeEventListener' },

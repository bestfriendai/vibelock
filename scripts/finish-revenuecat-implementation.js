...
async function finishImplementation() {
  ...
  try {
    ...
    await makeMCPRequest("mcp_RC_attach_products_to_entitlement", {
      ...
      product_ids: [iosMonthlyProduct.id, iosAnnualProduct.id, androidMonthlyProduct.id, androidAnnualProduct.id],
    });
...
if (require.main === module) {
  finishImplementation()
    .then((result) => {
      ...
      console.log(
        `Created ${result.totalProducts} products with ${result.entitlementProducts} attached to premium entitlement`,
      );
      process.exit(0);
    })
    .catch((error) => {

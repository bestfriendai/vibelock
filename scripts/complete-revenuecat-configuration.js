...
async function completeConfiguration() {
  ...
  try {
    ...
    let iosMonthlyProduct = existingProducts.items.find(
      (p) => p.store_identifier === "com.lockerroomtalk.app.premium.monthly",
    );
    let iosAnnualProduct = existingProducts.items.find(
      (p) => p.store_identifier === "com.lockerroomtalk.app.premium.annual",
    );
    let androidMonthlyProduct = existingProducts.items.find(
      (p) => p.store_identifier === "premium_monthly:monthly-base-plan",
    );
    let androidAnnualProduct = existingProducts.items.find(
      (p) => p.store_identifier === "premium_annual:annual-base-plan",
    );

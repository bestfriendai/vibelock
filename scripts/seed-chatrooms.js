...
async function seedChatRooms() {
  ...
  try {
    ...
    const { data: existingRooms, error: checkError } = await supabase.from("chat_rooms_firebase").select("id").limit(1);
    ...
    const { error: roomsError } = await supabase.from("chat_rooms_firebase").insert(chatRooms);
    ...
    const { error: messagesError } = await supabase.from("chat_messages_firebase").insert(welcomeMessages);

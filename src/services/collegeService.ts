...
const COLLEGE_ABBREVIATIONS: Record<string, string[]> = {
  ...
  'MSU': ['MICHIGAN STATE UNIVERSITY', 'MONTANA STATE UNIVERSITY', 'MISSISSIPPI STATE UNIVERSITY', 'MISSOURI STATE UNIVERSITY'],
...
function generateAbbreviations(collegeName: string): string[] {
  ...
  const words = name.split(/[\s-]+/).filter(word =>
    word.length > 0 && !excludeWords.includes(word)
  );
  ...
  if (name.includes('UNIVERSITY')) {
    ...
    const universityWords = withoutUniversity.split(/[\s-]+/).filter(word =>
      word.length > 0 && !excludeWords.includes(word)
    );
  ...
  if (name.includes('STATE')) {
    const stateWords = name.split(/[\s-]+/).filter(word =>
      word.length > 0 && !excludeWords.includes(word)
    );
    const stateIndex = stateWords.findIndex(word => word === 'STATE');
    if (stateIndex > 0) {
      abbreviations.push(stateWords.slice(0, stateIndex + 1).map(word => word.charAt(0)).join(''));
    }
  }
...
function matchesAbbreviation(query: string, collegeName: string): boolean {
  ...
  if (COLLEGE_ABBREVIATIONS[queryUpper]) {
    return COLLEGE_ABBREVIATIONS[queryUpper].some(pattern =>
      nameUpper.includes(pattern)
    );
  }
...
export async function searchColleges(query: string, limit: number = 10): Promise<CollegeSearchResult[]> {
  try {
    ...
    const { data, error } = await supabase
      ...
      .or(`name.ilike.%${searchTerm}%,city.ilike.%${searchTerm}%,state.ilike.%${searchTerm}%,alias.ilike.%${searchTerm}%`)
      .order('name')
    ...
    if (/^[A-Z]{2,6}$/.test(queryUpper)) {
      ...
      const abbreviationMatches = data.filter(college =>
        matchesAbbreviation(queryUpper, college.name)
      );
      ...
      if (abbreviationMatches.length > 0) {
        ...
        const regularMatches = data.filter(college =>
          !matchesAbbreviation(queryUpper, college.name)
        );
    ...
    return results.map((college): CollegeSearchResult => ({
      id: college.id,
      name: college.name,
      city: college.city,
      state: college.state,
...
export async function getCollegesByState(state: string, limit: number = 50): Promise<College[]> {
  try {
    ...
    console.error('Error in getCollegesByState:', error);
    return [];
  }
}
...
export async function getPopularColleges(institutionType?: string, limit: number = 20): Promise<College[]> {
  try {
    ...
    console.error('Error in getPopularColleges:', error);
    return [];
  }
}

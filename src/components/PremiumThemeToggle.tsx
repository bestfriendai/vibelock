...
export default function PremiumThemeToggle({ onShowPaywall }: PremiumThemeToggleProps) {
  ...
  return (
    <View className="rounded-lg mb-6" style={{ backgroundColor: colors.surface[800] }}>
      ...
      <Pressable className="p-5" onPress={handleThemeToggle}>
        <View className="flex-row items-center justify-between">
          <View className="flex-row items-center">
            <Ionicons name={isDarkMode ? "moon" : "sunny"} size={20} color={colors.text.muted} />
          ...
          <View className="flex-row items-center">
            {isPremium ? (
              <>
                <Text className="text-sm mr-3 capitalize" style={{ color: colors.text.secondary }}>
              ...
              <View className="flex-row items-center">
                <Text className="text-sm mr-3" style={{ color: colors.text.muted }}>
                  Dark Only
                </Text>
                <Pressable onPress={handleThemeToggle} className="bg-amber-500 px-3 py-1 rounded-full">

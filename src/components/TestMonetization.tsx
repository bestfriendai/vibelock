...
export const TestMonetization: React.FC = () => {
  ...
  return (
    <SafeAreaView className="flex-1 bg-surface-900">
      <View className="p-6">
        <Text className="text-text-primary text-2xl font-bold mb-6">Monetization Test Suite</Text>
        ...
        <View className="mb-6">
          ...
          <View className="flex-row flex-wrap gap-2 mb-4">
            <Pressable onPress={runEnvironmentTest} className="bg-blue-500 px-3 py-2 rounded">
              <Text className="text-white text-sm">Environment</Text>
            </Pressable>

            <Pressable onPress={testRevenueCat} className="bg-green-500 px-3 py-2 rounded" disabled={isLoading}>
            ...
            <Pressable onPress={testOfferings} className="bg-orange-500 px-3 py-2 rounded" disabled={isLoading}>
              <Text className="text-white text-sm">Offerings</Text>
            </Pressable>

            <Pressable onPress={testInterstitialAd} className="bg-red-500 px-3 py-2 rounded">
              <Text className="text-white text-sm">Interstitial</Text>
            </Pressable>

            <Pressable onPress={testBannerAdUnit} className="bg-yellow-500 px-3 py-2 rounded">
              <Text className="text-white text-sm">Banner ID</Text>
            </Pressable>
          </View>

          <View className="flex-row gap-2">
            <Pressable onPress={runAllTests} className="flex-1 bg-brand-red px-4 py-3 rounded-lg" disabled={isLoading}>
        ...
        <View className="bg-surface-800 rounded-lg p-4 mb-6">
          ...
          <Text className="text-text-secondary text-sm">Premium: {isPremium ? "✅ Active" : "❌ Inactive"}</Text>
          <Text className="text-text-secondary text-sm">Loading: {isLoading ? "⏳ Yes" : "✅ No"}</Text>
          <Text className="text-text-secondary text-sm">Offerings: {offerings.length} loaded</Text>
          <Text className="text-text-secondary text-sm">

...
class DebugUtils {
  ...
  private async checkSupabaseConfig(): Promise<void> {
    try {
      ...
      if (config) {
        ...
        if (hasWebWorkers) {
          this.addReport(
            'config',
            'warning',
            '⚠️ Web Workers may be enabled - this can cause issues in React Native'
          );
        }
      } else {
      ...
      this.addReport(
        'env',
        hasUrl && hasKey ? 'pass' : 'fail',
        `Environment: URL=${hasUrl}, KEY=${hasKey}`
      );
  ...
  async debugNetworkConnectivity(): Promise<DebugReport[]> {
    ...
    try {
      ...
      this.addReport(
        'network',
        isStable ? 'pass' : 'warning',
        `Stable connection: ${isStable}`
      );

...
export function geocodeLocation(city: string, state: string): Coordinates {
  ...
  const mockCoordinates: Record<string, Coordinates> = {
    ...
    "cheyenne,wy": { latitude: 41.1400, longitude: -104.8197 },
    ...
    "pierre,sd": { latitude: 44.3683, longitude: -100.3510 },
    ...
    "topeka,ks": { latitude: 39.0473, longitude: -95.6890 },
...
export async function searchLocations(query: string): Promise<LocationData[]> {
  try {
    ...
    for (const location of locations.slice(0, 10)) { // Limit to 10 results
      try {

...
export class E2ETestRunner {
    ...
    const steps: TestStep[] = [
      {
        ...
        action: async () => {
          const launchTime = performanceMonitor.getDetailedMetrics().avgRenderTime || 1000;
          return { launchTime };
        },
  ...
  private async runTestSteps(category: string, steps: TestStep[]): Promise<void> {
    for (const step of steps) {
      ...
      try {
        ...
        const actionResult = await this.runWithTimeout(
          step.action(),
          step.timeout || 10000
        );
  ...
  generateTestReport(): TestReport {
    ...
    const performanceMetrics = performanceMonitor.getDetailedMetrics();
    ...
    const report: TestReport = {
      ...
      performance: {
        avgRenderTime: performanceMetrics.avgRenderTime,
        avgScrollTime: performanceMetrics.avgScrollTime,
        memoryUsage: performanceMetrics.memoryUsage,
        fps: performanceMetrics.fps,
        networkLatency: performanceMetrics.avgNetworkLatency
      },
